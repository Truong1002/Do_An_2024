...
namespace ShopEcommerce.Admin.Users
{
                        ...
                        CreateUserDto, UpdateUserDto>, IUsersAppService
    {
        ...
        public async override Task<UserDto> CreateAsync(CreateUserDto input)
        {
            ...
            {
                throw new UserFriendlyException("Tài khoản đã tồn tại", ShopEcommerceDomainErrorCodes.AccountAlreadyExists);
            }

            var isUserEmailExisted = query.Any(x => x.Email == input.Email);
            if (isUserEmailExisted)
            {
                throw new UserFriendlyException("Email đã tồn tại", ShopEcommerceDomainErrorCodes.EmailAlreadyExists);
            }
